...
func registerOn(router *httprouter.Router, logger *log.Logger) {
	...
	router.GET("/resources/:resource/custom-rules.yaml", h.retrieveFalcoRulesForHelmChartHandlerLatestVersion)
	router.GET("/resources/:resource/custom-rules.yaml/:version", h.retrieveFalcoRulesForHelmChartHandler)
